...
export let createComment = async (req, res, next) => {
    try {
        ...
        if (userId !== req.user.id) {
            return next(errorHandler(403, "You are not allowed to create an comment"))
        }
...
export let getPostComments = async (req, res, next) => {
    try {
        let comments = await Comment.find({ postId: req.params.postId }).sort({ createdAt: -1 });
...
export let editComment = async (req, res, next) => {
    try {
        const comment = await Comment.findById(req.params.commentId)
        if (!comment)
            return next(errorHandler(404, "Comment not found"))
        if (comment.userId !== req.user.id && !req.user.isAdmin) {
            return next(errorHandler(403, "You are not alloweed to edit this comment"))
        }
...
export const deleteComment = async (req, res, next) => {
    try {
        ...
        if (!comment)
            return next(errorHandler(404, "Comment not found"));
        if (comment.userId !== req.user.id && !req.user.isAdmin) {
            return next(errorHandler(403, "You are not allowed not delete this comment"))
        }
